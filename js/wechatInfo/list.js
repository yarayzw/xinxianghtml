...
function setMaterial() {
    ajaxGo('admin/user/getUserListToSelect')
...
function initTable() {
    ...
    $("#viewTable").bootstrapTable({
        url: __ROOT__ + 'admin/wechat_config/getList', //获取数据的Servlet地址
        striped: true,  //表格显示条纹
...
function editView(obj) {
    ...
    $("input[name='annual_review']").val(requestData.data.annual_review);
    $("input[name='subject']").val(requestData.data.subject);
...
function del(obj) {
    layer.msg('确定删除？', {
        ...
        ,yes: function(index){
            ...
            ajaxGo('admin/wechat_config/delUserThirdpartyInfo');
            if(requestCode === 0){
                fac_search();
                layer.msg('删除成功!');
            }else {
