    ...
    $.ajax({
        ...
        success: function (data) {
            data.headInfo.forEach((item,index,array)=> {
                //顶部资讯
                let html = '<a href="http://'+goHttp+'/ex/zx'+'/index.html?id='+item.id+'&platform_id='+getUrlParam('platform_id')+'" style="display: block;position: relative;z-index:1;margin-bottom: 10px;">\n' +
            ...
            data.listInfo.forEach((item,index,array)=>{
                if(i % 4 === 0){
                    html = '<a href="http://'+goHttp+'/ex/zx'+'/index.html?id='+item.id+'&platform_id='+getUrlParam('platform_id')+'" style="display: block;margin:0 10px;"><h3 style="margin:10px">'+item.title+'</h3>\n' +
                        '\n' +
                    ...
                    html = '<div style="position: relative;z-index:1;padding:15px 10px;border-bottom: 1px solid #d7d7d7;" class="clearfix">\n' +
                        '            <a href="http://'+goHttp+'/ex/zx'+'/index.html?id='+item.id+'&platform_id='+getUrlParam('platform_id')+'"><img src="'+item.thumbnail_small+'" alt="" style="width:135px; height:80px; float:right">\n' +
